...
public class BioClipseConvenience {
	 ...
	 */
	public BioClipseConvenience(double mzabs, double mzppm, Spectrum spectrum, List<IMolecule> molecules, 
...
//		WrapperSpectrum spectrum = new WrapperSpectrum(this.peaks, mode, exactMass);
		...
		for (int c = 0; c < molecules.size(); c++) {
			...
			CDKManager cdkmanager = new CDKManager();;
			
			IAtomContainer molecule = cdkmanager.asCDKMolecule(molecules.get(c)).getAtomContainer();
			...
			Fragmenter fragmenter = new Fragmenter((Vector<Peak>) this.peaks.clone(), mzabs, mzppm, mode, true, this.sumFormulaRedundancyCheck, false, false);     
			...
			Vector<Peak> peakListParsed = this.peaks;
			...
			Scoring score = new Scoring(this.peaks);
